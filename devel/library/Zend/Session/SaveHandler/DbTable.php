...
class Zend_Session_SaveHandler_DbTable extends Zend_Db_Table_Abstract implements Zend_Session_SaveHandler_Interface {
	...
	public function gc($maxlifetime) {
		$this->delete ( $this->getAdapter ()->quoteIdentifier ( $this->_modifiedColumn ) . ' + ' . $this->getAdapter ()->quoteIdentifier ( $this->_lifetimeColumn ) . ' < ' . $this->getAdapter ()->quote ( time () ) );
		...
		foreach ( $this->_primary as $index => $primary ) {
			...
			switch (( string ) $type) {
				...
				case self::PRIMARY_TYPE_WHERECLAUSE :
					$primaryArray [] = $this->getAdapter ()->quoteIdentifier ( $primary ) . ' = ' . $this->getAdapter ()->quote ( $value );
					break;
