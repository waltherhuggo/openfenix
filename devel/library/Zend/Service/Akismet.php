...
class Zend_Service_Akismet extends Zend_Service_Abstract {
	...
	protected function _post($host, $path, array $params) {
		...
		$client->setHeaders ( array ('Host' => $host, 'Content-Type' => 'application/x-www-form-urlencoded; charset=' . $this->getCharset () ) );
	...
	public function verifyKey($key = null, $blog = null) {
		...
		$response = $this->_post ( 'rest.akismet.com', '/1.1/verify-key', array ('key' => $key, 'blog' => $blog ) );
