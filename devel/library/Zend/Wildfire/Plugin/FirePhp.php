	...
	protected function _recordMessage($structure, $data) {
		switch ($structure) {
			...
			case self::STRUCTURE_URI_DUMP :
				...
				return $this->_channel->getProtocol ( self::PROTOCOL_URI )->recordMessage ( $this, $structure, array ($data ['key'] => $this->_encodeObject ( $data ['data'] ) ) );
			...
			case self::STRUCTURE_URI_FIREBUGCONSOLE :
				...
				return $this->_channel->getProtocol ( self::PROTOCOL_URI )->recordMessage ( $this, $structure, array ($data ['meta'], $this->_encodeObject ( $data ['data'] ) ) );
