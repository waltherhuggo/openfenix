...
abstract class Zend_View_Helper_FormElement extends Zend_View_Helper_HtmlElement {
	...
	protected function _getInfo($name, $value = null, $attribs = null, $options = null, $listsep = null) {
		...
		// if it's an array.
		$info = array ('name' => is_array ( $name ) ? '' : $name, 'id' => is_array ( $name ) ? '' : $name, 'value' => $value, 'attribs' => $attribs, 'options' => $options, 'listsep' => $listsep, 'disable' => false, 'escape' => true );
		...
		if (isset ( $info ['attribs'] ['readonly'] ) && $info ['attribs'] ['readonly'] != 'readonly') {
			$info ['attribs'] ['readonly'] = 'readonly';
		}
		
		// Disable attribute
		if (isset ( $info ['attribs'] ['disable'] ) && is_scalar ( $info ['attribs'] ['disable'] )) {
			// disable the element
			$info ['disable'] = ( bool ) $info ['attribs'] ['disable'];
			unset ( $info ['attribs'] ['disable'] );
		} elseif (isset ( $info ['attribs'] ['disable'] ) && is_array ( $info ['attribs'] ['disable'] )) {
