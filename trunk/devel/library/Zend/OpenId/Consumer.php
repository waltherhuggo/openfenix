		...
		if ($this->_storage->getAssociationByHandle ( $params ['openid_assoc_handle'], $url, $macFunc, $secret, $expires )) {
			...
			if (base64_decode ( $params ['openid_sig'] ) == Zend_OpenId::hashHmac ( $macFunc, $data, $secret )) {
				...
				if (isset ( $params ['openid_claimed_id'] )) {
					...
					if (! Zend_OpenId::normalize ( $id ) || ! $this->_discovery ( $id, $discovered_server, $discovered_version ) || (! empty ( $params ['openid_identity'] ) && $params ["openid_identity"] != $id) || (! empty ( $params ['openid_op_endpoint'] ) && $params ['openid_op_endpoint'] != $discovered_server) || $discovered_version != $version) {
			...
			if (! Zend_OpenId::normalize ( $id ) || ! $this->_discovery ( $id, $server, $discovered_version )) {
