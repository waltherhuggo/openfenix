...
class Zend_Ldap {
	...
	public function setOptions(array $options) {
		$permittedOptions = array ('host' => null, 'port' => null, 'useSsl' => null, 'username' => null, 'password' => null, 'bindRequiresDn' => null, 'baseDn' => null, 'accountCanonicalForm' => null, 'accountDomainName' => null, 'accountDomainNameShort' => null, 'accountFilterFormat' => null, 'allowEmptyPassword' => null, 'useStartTls' => null, 'optReferrals' => null );
		...
		foreach ( $permittedOptions as $key => $val ) {
			if (! array_key_exists ( $key, $options )) {
				...
				switch ($key) {
					...
					case 'optReferrals' :
						$val = $options [$key];
						$options [$key] = $val === true || $val === '1' || strcasecmp ( $val, 'true' ) == 0;
						break;
				}
			}
		}
	...
	private function _getAccount($acctname, array $attrs = null) {
		...
		$result = @ldap_search ( $resource, $baseDn, $accountFilter, $attrs );
	...
	public function connect($host = null, $port = 0, $useSsl = false) {
		...
		if (is_resource ( $resource ) === true) {
			...
			if (@ldap_set_option ( $resource, LDAP_OPT_PROTOCOL_VERSION, 3 ) && @ldap_set_option ( $resource, LDAP_OPT_REFERRALS, $optReferrals )) {
				if ($useSsl || $this->_options ['useStartTls'] !== true || @ldap_start_tls ( $resource )) {
					return $this;
				}
			}
