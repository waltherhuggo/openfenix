...
class Zend_Service_SlideShare {
	...
	public function getCacheObject() {
		...
		if (! ($this->_cacheobject instanceof Zend_Cache_Core)) {
			$cache = Zend_Cache::factory ( 'Core', 'File', array ('lifetime' => 43200, 'automatic_serialization' => true ), array ('cache_dir' => '/tmp' ) );
	...
	public function getSlideShow($ss_id) {
		...
		$params = array ('api_key' => $this->getApiKey (), 'ts' => $timestamp, 'hash' => sha1 ( $this->getSharedSecret () . $timestamp ), 'slideshow_id' => $ss_id );
	...
	protected function _getSlideShowsByType($key, $value, $offset = null, $limit = null) {
		...
		$params = array ('api_key' => $this->getApiKey (), 'ts' => $timestamp, 'hash' => sha1 ( $this->getSharedSecret () . $timestamp ), $key => $value );
