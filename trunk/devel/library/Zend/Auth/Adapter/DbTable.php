...
class Zend_Auth_Adapter_DbTable implements Zend_Auth_Adapter_Interface {
	...
	protected function _authenticateCreateSelect() {
		...
		$credentialExpression = new Zend_Db_Expr ( '(CASE WHEN ' . $this->_zendDb->quoteInto ( $this->_zendDb->quoteIdentifier ( $this->_credentialColumn, true ) . ' = ' . $this->_credentialTreatment, $this->_credential ) . ' THEN 1 ELSE 0 END) AS ' . $this->_zendDb->quoteIdentifier ( 'zend_auth_credential_match' ) );
