...
;
	...
	public function toString($format = null, $type = null, $locale = null) {
		...
		for($i = 0; $i < strlen ( $format ); ++ $i) {
			...
			if (isset ( $output [$j] ) and ($output [$j] [0] == $format [$i]) or ($comment == true)) {
				$output [$j] .= $format [$i];
			} else {
				++ $j;
				$output [$j] = $format [$i];
	...
	public function toArray() {
		return array ('day' => $this->get ( self::DAY_SHORT ), 'month' => $this->get ( self::MONTH_SHORT ), 'year' => $this->get ( self::YEAR ), 'hour' => $this->get ( self::HOUR_SHORT ), 'minute' => $this->get ( self::MINUTE_SHORT ), 'second' => $this->get ( self::SECOND_SHORT ), 'timezone' => $this->get ( self::TIMEZONE ), 'timestamp' => $this->get ( self::TIMESTAMP ), 'weekday' => $this->get ( self::WEEKDAY_DIGIT ), 'dayofyear' => $this->get ( self::DAY_OF_YEAR ), 'week' => $this->get ( self::WEEK ), 'gmtsecs' => $this->get ( self::TIMEZONE_SECS ) );
	}
